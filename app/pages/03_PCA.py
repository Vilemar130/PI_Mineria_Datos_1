...
`customer_support_tickets` ≈ 0.62 en PC1, y `monthly_watch_time_mins` ≈ 0.89 en PC2)
- **PC1:** combinación de `age` + `customer_support_tickets` → "dimensión demográfico-soporte"
  - Puntuación alta = usuario mayor con más demanda de soporte técnico
- **PC2:** dominada por `monthly_watch_time_mins` → "dimensión de consumo"
...
st.markdown("""
**Interpretación:**
- **Por plan:** si `monthly_watch_time_mins` domina PC2 (ver loadings arriba), el desplazamiento
  de los usuarios Premium hacia mayor consumo debería verse en el eje **PC2**, no en PC1 — esto es
  coherente con el EDA (Visualización 3: Premium 1.085 min vs. Básico 589 min).
- **Por edad:** de igual forma, el gradiente por edad debería aparecer principalmente en el eje
  **PC1** (donde `age` tiene la carga más alta), con dispersión considerable dado que la
  correlación edad–consumo es prácticamente nula (EDA, Visualización 4).
- Revisar visualmente cuál eje separa mejor a los planes en el gráfico y ajustar esta
  interpretación si el signo/orden de los componentes difiere del de la tabla de loadings.
""")
